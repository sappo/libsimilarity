...
hmatrix_t *hmatrix_init(hstring_t *s, int n)
{
    ...
    hmatrix_t *m = (hmatrix_t *) zmalloc(sizeof(hmatrix_t));
    if (!m) {
    ...
    /* Allocate some space */
    m->labels = (float *) zmalloc(n * sizeof(float));
    m->srcs = (char **) zmalloc(n * sizeof(char *));
    if (!m->srcs || !m->labels) {
...
float *hmatrix_alloc(hmatrix_t *m)
{
    ...
    /* Allocate memory */
    m->values = (float *) zmalloc(sizeof(float) * m->size);
    if (!m->values) {
