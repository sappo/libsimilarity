...
if ! ((command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libconfig-dev >/dev/null 2>&1) || \
       (command -v brew >/dev/null 2>&1 && brew ls --versions libconfig >/dev/null 2>&1)); then
    $CI_TIME git clone --quiet --depth 1 https://github.com/hyperrealm/libconfig libconfig
    BASE_PWD=${PWD}
    cd libconfig
    CCACHE_BASEDIR=${PWD}
    export CCACHE_BASEDIR
...
if ! ((command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list zlib-dev >/dev/null 2>&1) || \
       (command -v brew >/dev/null 2>&1 && brew ls --versions zlib >/dev/null 2>&1)); then
    $CI_TIME git clone --quiet --depth 1 https://github.com/madler/zlib zlib
    BASE_PWD=${PWD}
    cd zlib
    CCACHE_BASEDIR=${PWD}
    export CCACHE_BASEDIR
...
if ! ((command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libarchive-dev >/dev/null 2>&1) || \
       (command -v brew >/dev/null 2>&1 && brew ls --versions libarchive >/dev/null 2>&1)); then
    $CI_TIME git clone --quiet --depth 1 https://github.com/libarchive/libarchive libarchive
    BASE_PWD=${PWD}
    cd libarchive
    CCACHE_BASEDIR=${PWD}
    export CCACHE_BASEDIR
