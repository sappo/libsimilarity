...
make install
cd ..
git clone --quiet --depth 1 https://github.com/hyperrealm/libconfig libconfig
cd libconfig
git --no-pager log --oneline -n1
if [ -e autogen.sh ]; then
    ./autogen.sh 2> /dev/null
...
make install
cd ..
git clone --quiet --depth 1 https://github.com/madler/zlib zlib
cd zlib
git --no-pager log --oneline -n1
if [ -e autogen.sh ]; then
    ./autogen.sh 2> /dev/null
...
make install
cd ..
git clone --quiet --depth 1 https://github.com/libarchive/libarchive libarchive
cd libarchive
git --no-pager log --oneline -n1
if [ -e autogen.sh ]; then
    ./autogen.sh 2> /dev/null
